...
def process_file(filename):
    ...
    class SectionNumberer:
        ...
        def __init__(self):
            ...
            self.bibliography_found = (
                False  # Flag to track if bibliography has been found
            )
        ...
        def replace_heading(self, match):
            ...
            if command == "bibliography":
                ...
                return match.group(
                    0
                )  # Optionally return the bibliography line unchanged
            ...
            if self.bibliography_found:
                if command == "section":
                    ...
                    section_label = chr(
                        64 + self.alpha_section_count
                    )  # Convert to letters A, B, C, etc.
                    ...
                    subsection_label = (
                        f"{chr(64 + self.alpha_section_count)}.{self.subsection_count}"
                    )
        ...
        for part in parts:
            if table_pattern.match(part):
                ...
                if caption_match:
                    ...
                    result_parts.append(
                        f"Table {caption_counter} Description: {caption_text}. End Table {caption_counter} Description."
                    )
                    caption_counter += 1
            else:
    ...
    def read_latex_doc(filename):
        ...
        def number_sections(tex_content):
            ...
            def replace_heading(match):
                nonlocal \
                    ...
                    appendix_mode, \
                    subsection_count
                heading_type = match.group(
                    1
                )  # Determine whether it's 'section' or 'subsection'
                ...
                if heading_type == "section":
                    ...
                    if appendix_mode:
                        subsection_count += 1
                        subsection_label = (
                            f"{chr(64 + alpha_section_count)}.{subsection_count}"
                        )
        ...
        list_chunks[0] = list_chunks[0].replace(
    ...
    for chunk in list_chunks:
        ...
        base_nodes.append(
            TextNode(
                text=chunk, id_=node_id, metadata={"license": check_license(chunk)}
            )
        )
    ...
    supporting_prompt_dict["B1"] = (
        """For composite artifacts like the GLUE benchmark, this means all creators. Cite the original paper that produced the code package or dataset. Remember to state which version of the asset you’re using."""
    )
    ...
    supporting_prompt_dict[
        "B3"
    ] = """For the artifacts you create, specify the intended use and whether that is compatible with the original access conditions (in particular, derivatives of data accessed for research purposes should not be used outside of research contexts).
    ...
    supporting_prompt_dict[
        "B5"
    ] = """Scientific artifacts may include code, data, models or other artifacts. Be sure to report the language of any language data, even if it is commonly-used benchmarks.
    ...
    supporting_prompt_dict["D3"] = (
        """For example, if the was collect via crowdsourcing, the instructions should explain to crowdworkers how the data would be used."""
    )
    ...
    ###
    prompt_dict["A1"] = (
        f"""Introduction: Behave like you are the author of a paper you are going to submit to a conference.
    ...
    ###
    prompt_dict["B1"] = (
        f"""Introduction: Behave like you are the author of a paper you are going to submit to a conference. Scientific artifacts may include code, data, models or other artifacts.
    ...
    # Save to JSON file
    with open("../aclready/src/components/Sidebar/sample_output.json", "w") as file:
        json.dump(results, file)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process a tex file.")
